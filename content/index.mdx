...
| 3 | **Developers write bad prompts** — Vague or incorrect prompts lead to wrong code, wasted time, and rework. Junior developers waste the most time. | Ships **49 pre-built skills** so developers don't write prompts from scratch — just run `/review`, `/security-check`, `/new-component`, `/refactor`, etc. |
...
Skills work in **both Claude Code and Cursor** — same 49 workflows, both tools.
...
- 49 skills in `.claude/skills/` + `.cursor/skills/` — auto-discovered workflows
...
| [Skills & Commands](/docs/slash-commands) | All 49 skills with usage guides |
