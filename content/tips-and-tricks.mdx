...
### Start Every AI Session with /kit-fetch-docs

Run `/kit-fetch-docs` at the start of each coding session to pre-load current API documentation for your tech stack. This is especially important for:
...
The generated `CLAUDE.md` also includes a "Documentation Verification" section that tells the AI to look up APIs when unsure — so even without `/kit-fetch-docs`, the AI will verify before guessing.
...
Even if you don't use the AI output directly, skills like `kit-review` and `kit-new-component` serve as checklists for what to cover. Invoke them explicitly with `/kit-review` in Claude Code, or let the AI auto-apply them when your request matches.
...
4. Use `/kit-understand` on unfamiliar modules
5. Use `/kit-prompt-help` when unsure how to ask
...
Use the **`/kit-effort`** command to control reasoning depth — lower effort saves tokens for simple tasks, higher effort produces better results for complex architecture decisions.

### Reduce Token Usage

1. **Be specific about files** — "Fix the error in `src/lib/auth.ts:45`" not "fix the auth"
2. **Use `/kit-understand` before modifying** — Costs fewer tokens than a failed attempt
...
5. **Use `/kit-effort` for simple tasks** — Lower reasoning effort = fewer tokens spent
