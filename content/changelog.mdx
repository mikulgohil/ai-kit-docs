...
| Agent | Purpose |
|---|---|
| `architect` | System design — rendering strategy, component hierarchy, data flow |
| `tdd-guide` | Test-driven development guide — red-green-refactor workflow |

Both agents are generated for every project. See [Agents](/docs/agents) for details.
...
### Sitecore Content SDK v2.x Support

- **New CMS type**: `sitecore-xmc-v2` — detected via `@sitecore-content-sdk/nextjs` in deps
- **Updated scanner**: Detection priority is now Content SDK v2.x → JSS Next.js → JSS React
- **Updated agent**: `sitecore-specialist` now covers Content SDK v2.x patterns, Experience Edge connectivity, and image optimization
- **Updated skill**: `/sitecore-debug` adds checks for Content SDK v2.x migration issues, Experience Edge connectivity, and image optimization

### Expanded Templates

- Next.js App Router — Server Action and middleware scaffolding skills
- Sitecore XM Cloud — Content SDK v2.x fragment with new SDK patterns
- TypeScript — stricter Zod validation patterns in generated skills
