...
Agents are specialized AI assistants in `.claude/agents/` that handle delegated tasks. Each has focused expertise, specific tools, and structured workflows. You invoke them with `@kit-agent-name` in Claude Code.
...
| `kit-code-reviewer` | Deep code review for quality and patterns |
...
| `kit-architect` | System design — SSR/SSG/ISR, component hierarchy |
| `kit-tdd-guide` | Test-driven development guidance |
| `kit-ci-debugger` | CI/CD failure analysis and resolution |
| `kit-data-scientist` | ML pipelines, model evaluation, data analysis |
| `kit-performance-profiler` | Core Web Vitals, bundle analysis, profiling |
| `kit-migration-specialist` | Framework upgrades, breaking changes, codemods |
| `kit-dependency-auditor` | Vulnerability scanning, license compliance |
| `kit-api-designer` | REST/GraphQL API design, schema validation |
| `kit-e2e-runner` | Playwright tests (conditional) |
| `kit-sitecore-specialist` | Sitecore XM Cloud patterns (conditional) |
...
Agents are a Claude Code feature (the `@kit-agent-name` delegation pattern). Cursor doesn't support agent delegation in the same way, but the agent markdown files can still be referenced as context.
...
Example: Use the `/kit-review` skill to review code yourself with AI guidance. Use `@kit-code-reviewer` to delegate the entire review to the agent.
