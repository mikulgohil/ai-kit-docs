...
Slash commands (`.claude/commands/*.md`) are a Claude Code feature and don't work in Cursor natively. However, **skills do work in Cursor**. AI Kit generates `.cursor/skills/` with the same 26 workflows. The AI in Cursor reads skill descriptions and auto-applies them when your task matches — so Cursor users get the full workflow library without typing slash commands.

### Can I add my own skills?

Yes. Create a subdirectory with a `SKILL.md` file in `.claude/skills/` and `.cursor/skills/`. AI Kit won't touch directories it didn't create:

```
.claude/skills/
├── review/SKILL.md        # AI Kit (updated on ai-kit update)
├── new-component/SKILL.md # AI Kit (updated on ai-kit update)
...
For Claude Code only, you can also still add legacy slash commands as `.md` files to `.claude/commands/`.
