...
No. Use `npx @horizontal/ai-kit init` to run it without installing. The tool only needs to run once per project (or when you want to update configs).
...
AI Kit still generates the **base rules** (Horizontal coding standards, naming conventions, testing, docs, etc.) which apply to any project. Framework-specific fragments are only added when detected.
...
```bash
npx @horizontal/ai-kit reset   # Remove all generated files
npx @horizontal/ai-kit init    # Re-generate from scratch
```
