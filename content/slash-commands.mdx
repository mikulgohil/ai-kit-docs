...
AI Kit generates 45 skills across two formats — **skills** for auto-discovery, and **legacy commands** for backward compatibility.
...
| `/docker-debug` | Maintenance | Analyzes Dockerfiles for security and efficiency |
| `/ci-fix` | Maintenance | Debugs CI/CD pipeline failures and optimizations |
| `/db-migrate` | Maintenance | Creates safe database migrations with rollback plans |
| `/dependency-graph` | Maintenance | Maps module dependencies and finds circular imports |
...
| `/visual-diff` | Workflow | Visual regression testing plan for UI changes |
| `/learn` | Session | Extracts reusable patterns from the current coding session |
...
Analyzes Dockerfiles and docker-compose configurations for security risks, build inefficiencies, image size issues, and best practice violations.

**Usage**
```
/docker-debug
/docker-debug Dockerfile.production
```

**When to use**: When building or optimizing Docker images, debugging container issues, or reviewing container security.

**What it checks**
- Multi-stage build opportunities
- Layer caching optimization
- Security issues (running as root, exposed secrets)
...
Debugs CI/CD pipeline failures, identifies caching opportunities, and suggests performance optimizations for GitHub Actions and other CI systems.
...
**When to use**: When a CI pipeline is failing, running slowly, or needs optimization.

**What it checks**
- Failure root cause analysis
- Caching configuration (node_modules, build cache)
- Parallel job opportunities
- Security best practices (pinned actions, scoped permissions)
- Missing steps (lint, test, build verification)

---

### /db-migrate

Creates safe database migrations with rollback strategies for Prisma, Drizzle, or raw SQL, with data loss risk assessment and backward compatibility checking.

**Usage**
```
/db-migrate add column email to users
/db-migrate rename column name to full_name in customers
```

**When to use**: When making schema changes, before deploying database modifications, or when planning zero-downtime migrations.

**What it covers**
- Data loss risk assessment
- Backward compatibility analysis
- Forward and rollback migration code
- Index and foreign key recommendations
- Multi-step strategy for destructive changes

---

### /dependency-graph

Maps module import relationships, finds circular dependencies, analyzes coupling metrics, and identifies architectural boundary violations.

**Usage**
```
/dependency-graph src/features/
/dependency-graph src/components/
```

**When to use**: When the codebase feels tangled, during architecture reviews, or when planning a refactoring initiative.

**What it analyzes**
- Circular dependency detection (direct and indirect)
- Afferent/efferent coupling metrics and instability ratios
- Cross-feature import violations
- Hub modules (high-risk bottlenecks)
...
Creates a visual regression test plan for changed components, mapping impact radius and identifying specific states and breakpoints to check.

**Usage**
```
/visual-diff
/visual-diff src/components/Button.tsx
```

**When to use**: After making UI changes, before merging visual PRs, or during QA review.

**What it covers**
- Changed component identification
- Impact radius mapping (parent and consumer components)
- Visual test matrix (states x breakpoints)
- CSS side effect warnings
...
Extract reusable patterns from the current coding session.

**When to use**: At the end of a productive session to capture what worked, what failed, and what patterns emerged.

**What it does**:
1. Reviews session activity — files created, modified, deleted
2. Extracts patterns with confidence ratings (high/medium/low)
3. Categorizes as: code patterns, debugging insights, architecture decisions, anti-patterns, tool usage
4. Saves findings to `ai-kit/learnings/[date]-[topic].md`
5. Suggests promoting high-confidence patterns to permanent project rules
