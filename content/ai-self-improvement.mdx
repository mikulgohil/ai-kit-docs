...
Developer A on Project X (Claude Code) gets a security report in the same format as Developer B on Project Y (Cursor). Whether triggered by `/kit-security-check` or auto-applied when the AI detects a security review task, both get:
...
- `/kit-pre-pr` checks the same 10 categories on every project
- `/kit-review` evaluates the same criteria everywhere
- `/kit-security-check` scans for the same vulnerability classes
...
| Code review feedback | Reviewer catches issues | `/kit-pre-pr` catches them before review |
