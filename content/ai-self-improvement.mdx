...
## Feature 3: Structured Command Output

Every slash command produces output in a **consistent, predictable format** — regardless of which developer runs it, on which project, at what time.

### How It Works

Each of the 25 slash commands has:
...
Developer A running `/security-check` on Project X gets a report in the same format as Developer B running `/security-check` on Project Y. Both get:
...
**Consistency across developers, projects, and time.**
...
3. **Structured Commands** — Each `.md` file in `.claude/commands/` uses prompt engineering patterns (role framing, mandatory steps, output templates, self-checks, constraints) to produce consistent output
