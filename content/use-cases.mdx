...
```
📋 ai-kit v1.11.0 | Stack: Next.js 16.2 (app router) + tailwind
...
Shows daily, weekly, and monthly token usage with cost estimates. Budget alerts at 50%, 75%, and 90% thresholds. Per-project breakdown so you can see which project consumes the most. Includes model recommendations — "Switch to Sonnet for /review tasks to save 40% with similar quality."
...
✓ ai-kit.config.json found (v1.11.0)
✓ Version matches CLI (v1.11.0)
...
```
@planner Plan the multi-city trip planner feature
```
...
| Agent | When to use |
|---|---|
| `@code-reviewer` | Deep quality review of specific files |
| `@security-reviewer` | Security-focused audit of auth flows, API routes |
| `@architect` | SSR vs SSG decisions, component hierarchy, data flow design |
| `@sitecore-specialist` | XM Cloud component mapping, GraphQL queries, Experience Edge |
| `@build-resolver` | Build is broken and you can't figure out why |
| `@e2e-runner` | Generate Playwright tests for a user flow |
| `@tdd-guide` | Red-green-refactor workflow guidance |
| `@performance-profiler` | Core Web Vitals deep dive, runtime profiling |
| `@migration-specialist` | Framework upgrade planning and execution |
| `@dependency-auditor` | Full dependency health check |
| `@ci-debugger` | CI pipeline is failing and logs are unclear |
| `@doc-updater` | Docs are out of sync with code changes |
| `@refactor-cleaner` | Find and remove dead code across the project |
| `@data-scientist` | Data analysis patterns, ML pipeline design |
...
- `@security-reviewer` audits auth flows and API routes
- `@e2e-runner` analyzes test coverage gaps
- `@performance-profiler` checks Core Web Vitals
...
1. **Manual invocation** — type `/review` or `/fix-bug` directly
...
| **Isolation** | Runs in your working directory | Some run in isolated worktrees (refactor-cleaner, build-resolver, e2e-runner) |
| **Example** | `/review` runs a checklist on files | `@code-reviewer` is a specialized reviewer persona that thinks like a senior engineer |

**Worktree isolation** (agents only): Agents like `@refactor-cleaner` and `@build-resolver` run in temporary git worktrees. They can safely modify files without affecting your working directory. If they break something, the worktree is discarded — your code is untouched.
