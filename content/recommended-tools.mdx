...
| `/kit-responsive-check` | AI takes actual screenshots at each breakpoint instead of only reading code |
| `/kit-accessibility-audit` | AI can inspect rendered output — not just source — for accessibility issues |
| `/kit-fix-bug` | AI can reproduce UI bugs in a real browser to confirm the fix works |
...
- `/kit-figma-to-code` receives exact spacing, color, typography, and component data from the Figma file — not your approximation of it
...
- Design handoff becomes a single step: share the Figma link, run `/kit-figma-to-code`
...
|---|---|
| `/kit-figma-to-code` | Reads exact design values instead of relying on your description |
| `/kit-design-tokens` | Compares Figma token names directly against your Tailwind config |
| `/kit-responsive-check` | Can read Figma frame dimensions to verify implementation matches spec |
| `/kit-new-component` | Component scaffolding can be seeded with real design values |
...
|---|---|
| `/kit-migrate` | Fetches real migration guides and breaking changes for the target library version |
| `/kit-new-component` | Generates code against current API docs, not potentially outdated training data |
| `/kit-api-route` | Uses current Next.js route handler API patterns |
| `/kit-type-fix` | Can look up current type definitions when TypeScript errors relate to library types |
| `/kit-dep-check` | Can fetch current changelog data when evaluating upgrade impact |
...
Without GitHub MCP, `/kit-pre-pr` runs its checklist and tells you what to do. With GitHub MCP, it can also create the PR for you, pre-filled with a structured description based on what it found. Without GitHub MCP, `/kit-review` can only review the code you paste or the files you point it at. With GitHub MCP, it can read the actual diff of an open PR.
...
|---|---|
| `/kit-pre-pr` | Can automatically create the PR after the checklist passes |
| `/kit-review` | Can read the actual diff of an open PR directly from GitHub |
| `/kit-commit-msg` | Can read recent commit history and open issues for better context |
| `/kit-fix-bug` | Can read the issue description and related comments for bug context |
...
- `/kit-migrate` can search for real migration experiences, gotchas, and community-reported issues
- `/kit-fix-bug` can search for known bugs related to the error message you are seeing
- `/kit-dep-check` can check for recent security advisories and community-reported vulnerabilities
...
|---|---|
| `/kit-migrate` | Searches for community migration reports, not just official docs |
| `/kit-fix-bug` | Searches for known issues matching the specific error message |
| `/kit-dep-check` | Searches for recent CVEs, advisories, and compatibility reports |
| `/kit-security-check` | Can research recent attack vectors and known vulnerability patterns |
...
**How it pairs with `/kit-new-component`:**

When you run `/kit-new-component`, the AI asks whether to generate a Storybook story. If you answer yes, it generates a `.stories.tsx` file alongside the component with stories for the primary use cases, loading state, error state, and edge cases — so you can review the component visually before wiring it into a page.
...
**How it pairs with `/kit-dep-check`:**

`/kit-dep-check` audits your dependencies for outdated versions, vulnerabilities, and sizing issues. Knip tells it exactly which packages are actually unused — so when `/kit-dep-check` recommends removing a package, it is backed by real data showing the package is not imported anywhere. Run Knip before running `/kit-dep-check` for the most accurate analysis.
...
**How it pairs with `/kit-security-check`:**

`/kit-security-check` analyzes your code for security vulnerabilities: XSS vectors, unvalidated inputs, exposed secrets, missing auth guards. Snyk complements this by checking your dependency tree for known CVEs. The two operate at different layers. Run `/kit-security-check` on your code, run `snyk test` on your dependencies. Both pass before a release.
...
**How it pairs with `/kit-optimize`:**

`/kit-optimize` analyzes your code and recommends performance improvements: add `useMemo` here, move this to a Server Component, lazy load this module. Lighthouse CI measures the actual result. Run `/kit-optimize` to get the recommendations, implement them, run `npm run lighthouse` to verify that Core Web Vitals scores improved. The audit loop closes.
...
`/kit-dep-check` identifies packages that could be removed or replaced. Bundle analyzer shows you the weight cost of each package — so when `/kit-dep-check` flags a large dependency, you can see exactly how much bundle weight removing it would save. Run `/kit-dep-check` first for the list, then run `npm run analyze` to quantify the impact.
...
| `/kit-new-component` | | Figma | Context7 | | | | | Storybook | ESLint | Prettier | | | | |
| `/new-page` | | | Context7 | | | | | | ESLint | Prettier | | | | |
| `/kit-api-route` | | | Context7 | | | | | | ESLint | Prettier | | Snyk | | |
| `/error-boundary` | | | Context7 | | | | | | ESLint | Prettier | | | | |
| `/kit-extract-hook` | | | Context7 | | | Playwright | | | ESLint | Prettier | | | | |
| `/kit-figma-to-code` | | Figma | | | | | | | ESLint | Prettier | | | | |
| `/kit-design-tokens` | | Figma | Context7 | | | | | | | | | | | |
| `/kit-review` | | | Context7 | GitHub | | | | | ESLint | Prettier | | | | |
| `/kit-pre-pr` | | | | GitHub | | | | | ESLint | Prettier | Knip | Snyk | | |
| `/test` | Playwright | | Context7 | | | Playwright | axe-core | Storybook | | | | | | |
| `/kit-accessibility-audit` | Playwright | | | | | | axe-core | | ESLint | | | | Lighthouse CI | |
| `/kit-security-check` | | | | | Perplexity | | | | ESLint | | | Snyk | | |
| `/kit-responsive-check` | Playwright | Figma | | | | Playwright | | | | | | | | |
| `/kit-type-fix` | | | Context7 | | | | | | ESLint | | | | | |
| `/kit-fix-bug` | Playwright | | Context7 | GitHub | Perplexity | Playwright | | | | | | | | |
| `/kit-refactor` | | | Context7 | | | | | | ESLint | Prettier | Knip | | | |
| `/kit-optimize` | | | Context7 | | Perplexity | | | | | | Knip | | Lighthouse CI | Bundle Analyzer |
| `/kit-migrate` | | | Context7 | | Perplexity | | | | ESLint | Prettier | | | | |
| `/kit-dep-check` | | | Context7 | | Perplexity | | | | | | Knip | Snyk | | Bundle Analyzer |
| `/kit-sitecore-debug` | Playwright | | Context7 | | Perplexity | | | | | | | | | |
| `/kit-document` | | | | GitHub | | | | Storybook | | | | | | |
| `/kit-commit-msg` | | | | GitHub | | | | | | | | | | |
| `/kit-env-setup` | | | | GitHub | | | | | | | | | | |
...
1. **Context7 MCP** — enhances 18 of 48 skills. Install this first.
