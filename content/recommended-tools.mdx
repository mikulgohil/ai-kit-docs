...
AI Kit works on its own — run `npx ai-kit init` on any project and you immediately get a smarter AI assistant. But a set of free, well-supported tools can unlock the full potential of every slash command. This page covers all of them: what each tool does, why it matters, how to set it up, and exactly which AI Kit commands it makes better.
...
Context7 MCP benefits every command that generates code. It is one of the highest-value MCPs in this list.
...
Notion MCP gives the AI read and write access to your Notion workspace. It can read page content, query databases, create new pages, and update existing ones.

For teams that use Notion as a project wiki or knowledge base, this turns the AI into a bridge between your code and your documentation. Component specifications written in Notion can inform how the AI builds them. Documentation generated by `/document` can be pushed to Notion automatically.

**What it enables:**

- `/document` can create or update a component page in your Notion wiki directly
- The AI can read project specifications from Notion before starting implementation
- The AI can query a Notion database to find which components already have documentation
- Meeting notes, design decisions, and project context stored in Notion become accessible during development
...
3. Give it a name (e.g., "AI Kit Integration"), select your workspace, and click **Submit**
4. Copy the **Internal Integration Token** shown on the next screen

Connect the integration to pages it should be able to access:

1. Open any Notion page you want the AI to be able to read or write
2. Click the **...** menu (top right)
3. Go to **Connections** > **Add connections**
4. Find and add the integration you just created
5. Repeat for each page or parent page
...
Ask the AI: "List the pages in my Notion workspace." If the MCP is active and the integration is connected, it will return the pages it has access to.

**Which AI Kit commands benefit:**

| Command | How Notion MCP helps |
|---|---|
| `/document` | Can create or update component documentation pages directly in Notion |
| `/understand` | Can read project specifications from Notion to inform code explanations |
| `/new-component` | Can read component specs from Notion before scaffolding |
...
### 9. axe-core and @axe-core/playwright
...
## Section 6: Tool and Command Pairing Matrix

This table maps every AI Kit slash command to the tools that meaningfully enhance it. "Core" means the command works fine without the tool. "Enhanced" means the tool unlocks additional capability or verification.

| AI Kit Command | Playwright MCP | Figma MCP | Context7 MCP | GitHub MCP | Perplexity MCP | Playwright Tests | axe-core | Storybook | ESLint | Prettier | Knip | Snyk | Lighthouse CI | Bundle Analyzer |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `/prompt-help` | | | | | | | | | | | | | | |
...
A cell with a tool name means that tool meaningfully improves that command. Empty cells mean the command works on its own without enhancement from that tool. No command requires all tools — install what fits your workflow and leave the rest for later.

**Highest-value tools by breadth of impact:**

1. **Context7 MCP** — enhances 17 of 25 commands. Install this first.
2. **ESLint** — enforces generated code quality across 14 commands. Install early.
3. **Playwright MCP** — enables browser automation for 8 commands. High value for QA-focused work.
4. **GitHub MCP** — automates PR and issue workflows for 6 commands. High value for teams.
5. **Perplexity MCP** — adds real-time research to 7 commands. High value for migration and debugging work.
